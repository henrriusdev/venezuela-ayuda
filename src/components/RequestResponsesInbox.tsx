  ...
  useEffect(() => {
    ...
    fetchRequestResponses(requestId, token).then((res) => {
      ...
      if (res.ok) {
        ...
        setError(res.error ?? "No se pudieron cargar las respuestas.");
      }
    });
    return () => {
      active = false;
  ...
  return (
    <section className="mt-4 rounded-2xl border border-[#e6ecf2] bg-white p-4">
      <h2 className="font-semibold text-[#14212e]">
        Respuestas de voluntarios ({count})
      </h2>
      ...
      {error ? (
        ...
        <p className="mt-2 text-sm text-[#8190a0]">Aún no hay respuestas.</p>
      ) : (
